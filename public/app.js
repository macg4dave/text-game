...
(() => {
  ...
  var logEl = getElement("log");
  var formEl = getElement("input-form");
  var nameEl = getElement("player-name");
  var inputEl = getElement("player-input");
  var sendButtonEl = getElement("send-button");
  var optionsEl = getElement("options");
  var assistEl = getElement("assist");
  var statusPillEl = getElement("status-pill");
  var preflightPanelEl = getElement("preflight-panel");
  var preflightTitleEl = getElement("preflight-title");
  var preflightSummaryEl = getElement("preflight-summary");
  var preflightIssuesEl = getElement("preflight-issues");
  var runtimeSummaryEl = getElement("runtime-summary");
  var sessionSummaryEl = getElement("session-summary");
  var connectionDebugEl = getElement("connection-debug");
  var stateDebugEl = getElement("state-debug");
  var turnDebugEl = getElement("turn-debug");
  var refreshSessionButtonEl = getElement("refresh-session");
  var newSessionButtonEl = getElement("new-session");
  var state = {
  ...
  function renderPreflightPanel() {
    ...
    (preflight.issues || []).forEach((issue) => {
      ...
      const parts = [issue.title, issue.message].filter((value) => Boolean(value));
      const recovery = Array.isArray(issue.recovery) ? issue.recovery.filter(Boolean) : [];
      item.textContent = recovery.length ? `${parts.join(": ")} Fix: ${recovery.join(" ")}` : parts.join(": ");
  ...
  formEl.addEventListener("submit", async (event) => {
    ...
    try {
      ...
      const latency = typeof result.data.debug?.turn?.latency_ms === "number" ? result.data.debug.turn.latency_ms : void 0;
