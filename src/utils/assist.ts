import type { AssistCorrection, AssistResponse } from "../core/types.js";
...
export function assistText({ text, dynamicTexts = [] }: { text: string; dynamicTexts?: string[] }): AssistResponse {
...
function buildVocabulary(baseWords: readonly string[], dynamicWords: string[], maxSize: number): string[] {
...
function spellcheck(text: string, vocab: string[], maxSuggestions: number): AssistCorrection[] {
  if (!text) return [];
  const tokens = text.match(/[A-Za-z']+/g) || [];
  const vocabSet = new Set(vocab);
  const corrections: AssistCorrection[] = [];
...
function autocomplete(text: string, vocab: string[], limit: number): string[] {
  if (!text) return [];
  ...
  return vocab.filter((word) => word.startsWith(prefix) && word.length > prefix.length).slice(0, limit);
}

function getClosestWords(token: string, vocab: string[], limit: number): string[] {
  const first = token[0];
  if (!first) return [];
