...
| Repo automation lives in the `launcher/` Rust executable `SunRay`, and legacy shell-script orchestration is retired as a supported path | Locked | Tech lead | The script surface outgrew demo-grade shell wrappers; `SunRay` gives one structured implementation language for launcher, harness, smoke, and validation commands without rewriting Docker, installers, or the app runtime. | Start of `T66` |
