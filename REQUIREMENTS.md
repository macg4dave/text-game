...
- The opening playable slice must support server-owned authored scene grounding for common exploratory turns such as `look around`, `inspect <thing>`, short referential follow-ups, and simple topical questions, with visible landmarks, named actors, and obvious exits surfaced clearly enough that limited local models do not collapse those turns into generic lead-reminder output.
- When an opening-slice exploratory turn commits no state change, the returned narration should still prefer grounded authored scene detail or nearby route guidance when available instead of falling back to the same generic lead-reminder loop.
- Dialogue input should accept both parser-like commands and natural first-person or quoted speech. Inputs such as `I ask Nila where the power comes from` or `"Where does it draw power from?"` should count as in-world dialogue attempts when context makes the target clear, rather than being treated as generic clarification or out-of-band meta text.
...
- Early movement should feel local and legible: nearby authored destinations and aliases in the opening hub should accept natural phrasing, short directional inputs such as `go down` should resolve or clarify against authored nearby routes, and blocked or premature travel should return grounded direction instead of a generic no-change fallback.
