...
- the default launcher keeps the normal `litellm.config.yaml` alias routing and adds GPU reservation to Ollama
- `litellm.local-gpu.config.yaml` is now a manual larger-model experiment file instead of the default launcher config
...
- `litellm.local-gpu.config.yaml` for manual Docker-backed larger-model experiments

The included `litellm.local-gpu.config.yaml` keeps `local-gpu-8gb` active and includes commented manual swap references for `local-gpu-12gb` and `local-gpu-20gb-plus`. Use it only when you intentionally want to experiment beyond the default launcher path.
...
- `AI_PROFILE=custom` keeps startup diagnostics aligned with the direct-provider smoke-test path instead of reporting local GPU profile defaults plus advanced overrides.
...
2. Use the GPU-backed Docker stack for the repo-managed Docker Ollama route.
3. Use `litellm.local-gpu.config.yaml` only when you intentionally want larger-model manual experiments beyond the default launcher path.
...
- if the selected Ollama model is missing, startup now reports that the local model must be pulled before retrying
- if the launcher cannot find `nvidia-smi`, it now blocks immediately because the default launcher path assumes GPU-backed Docker inference
- if Docker does not report an `nvidia` runtime, the launcher now blocks immediately instead of allowing a quiet CPU fallback
...
- The gateway-aligned local-model path now assumes the GPU-backed Docker launcher contract.
...
- Small local models may drift from the JSON schema more often than the default LiteLLM path.
